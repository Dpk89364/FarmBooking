package com.example.farmbooking;

import androidx.appcompat.app.AppCompatActivity;

import android.os.Bundle;
import android.widget.ImageView;
import android.widget.TextView;

import com.squareup.picasso.Picasso;

public class Report extends AppCompatActivity {

    String imgURL="data:image/jpeg;base64,";

    ImageView dialog_imageview;
    TextView text_view_h0,text_view_h1,text_view_h2,text_view_h3,text_view_h4,text_view_h5,text_view_h6,text_view_h7,text_view_h8;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_report);

        dialog_imageview=findViewById(R.id.dialog_ImageView);
        text_view_h0=findViewById(R.id.text_view_h0);
        text_view_h1=findViewById(R.id.text_view_h1);
        text_view_h2=findViewById(R.id.text_view_h2);
        text_view_h3=findViewById(R.id.text_view_h3);
        text_view_h4=findViewById(R.id.text_view_h4);
        text_view_h5=findViewById(R.id.text_view_h5);
        text_view_h6=findViewById(R.id.text_view_h6);
        text_view_h7=findViewById(R.id.text_view_h7);
        text_view_h8=findViewById(R.id.text_view_h8);


        String Price=getIntent().getExtras().get("PR").toString();
        String Size=getIntent().getExtras().get("S").toString();
        String Type=getIntent().getExtras().get("T").toString();
        String Address=getIntent().getExtras().get("A").toString();
        String Name=getIntent().getExtras().get("N").toString();
        String Phone=getIntent().getExtras().get("P").toString();
        String Email=getIntent().getExtras().get("E").toString();
        String Landmark=getIntent().getExtras().get("L").toString();
        String Pincode=getIntent().getExtras().get("PIN").toString();

        text_view_h0.setText("Price: "+Price + "$");
        text_view_h1.setText("Size: "+Size + " sq. ft");
        text_view_h2.setText("Type: "+Type);
        text_view_h3.setText("Address: "+Address);
        text_view_h4.setText("Landmark: "+Landmark);
        text_view_h5.setText("Pincode: "+Pincode);
        text_view_h6.setText("Name: "+Name);
        text_view_h7.setText("Phone: "+Phone);
        text_view_h8.setText("Email: "+Email);

        Picasso.with(this).load(imgURL).placeholder(R.drawable.rent_dummy).fit().centerCrop().into(dialog_imageview);
    }
}